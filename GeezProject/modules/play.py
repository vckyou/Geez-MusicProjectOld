...
async def generate_cover(requested_by, title, views, duration, thumbnail):
    ...
    draw.text((50, 1000), f"Now playing", (10000, 10000, 10000), font=font)
    draw.text(
        (50, 1000), f"{title}", (5000, 5000, 5000), font=font
    )
    img.save("final.png")
    os.remove("temp.png")
    os.remove("background.png")
...
async def play(_, message: Message):
    ...
    if audio:
        ...
        query = toxt
        await lel.edit("`I process your song request...`")
...
async def ytplay(_, message: Message):
    ...
    for i in message.command[1:]:
        query += " " + str(i)
    print(query)
    await lel.edit("`Song in progress`")
    ydl_opts = {"format": "bestaudio[ext=m4a]"}
    try:
        results = YoutubeSearch(query, max_results=1).to_dict()
        url = f"https://youtube.com{results[0]['url_suffix']}"
...
async def deezer(client: Client, message_: Message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [InlineKeyboardButton(text="ᴄʜᴀɴɴᴇʟ ɪɴғᴏ", url="https://t.me/rizexproject")],
        ]
    )
